...
//    }
  ...
  bool isWithinListView(Offset dragPosition) {
    final RenderBox renderBox =
        _wrapKey.currentContext!.findRenderObject() as RenderBox;
    final Offset listViewPosition = renderBox.localToGlobal(Offset.zero);
    final Size listViewSize = renderBox.size;
    print("---listViewPosition---- ${listViewPosition.toString()}");
    print("---listViewSize---- ${listViewSize.toString()}");

    final bool isWithinHorizontalBounds =
        dragPosition.dx >= listViewPosition.dx &&
            dragPosition.dx <= (listViewPosition.dx + listViewSize.width);
    final bool isWithinVerticalBounds =
        dragPosition.dy >= listViewPosition.dy &&
            dragPosition.dy <= (listViewPosition.dy + listViewSize.height);
...
//        debugPrint('onDragStarted: index:$index _ghostDisplayIndex:$_ghostDisplayIndex _currentDisplayIndex:$_currentDisplayIndex _dragStartIndex:$_dragStartIndex');
    ...
    void onDragUpdate(DragUpdateDetails detail) {
      final Offset dragPosition = detail.globalPosition;
      final bool withinBounds = isWithinListView(dragPosition);

      print("------- ${dragPosition.toString()}");
      if (withinBounds) {
        // The drag is inside the ListView
        setState(() {
          _placeholderIndex = index;
        });
      } else {
        // The drag is outside the ListView
        setState(() {
          _placeholderIndex = null;
        });
      }

      // widget.onDragUpdate?.call(detail);
    }
